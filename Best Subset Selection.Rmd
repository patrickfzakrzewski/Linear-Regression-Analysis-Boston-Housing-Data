...
for(k in 1:10){
  best.fit=regsubsets(Salary~.,data=Hitters[fold!=k,],nvmax=19,method="forward")
  for(i in 1:19){
...
y=Hitters$Salary
```
First we will fit a ridge-regression model. This is achieved by calling `glmnet` with `alpha=0` (alpha=0 is ridge, alpha=1 is lasso). There is also a `cv.glmnet` function which will do the cross-validation for us.
```{r}
...
```{r}
fit.lasso=glmnet(x,y) #did not specify alpha because default for alpha is 1
